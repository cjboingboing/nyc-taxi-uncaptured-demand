...
Calum Moir \\
Student ID: 1352920 \\
\href{https://github.com/cjboingboing/nyc-taxi-uncaptured-demand}{Github repo with commit}
}
...
2025 is used as the modelling year throughout: the most recent complete year of data, chosen over the COVID-distorted window for a realistic
picture of the market as it stands today. The TLC Trip Records dataset~\cite{tlc_trip_data} forms the backbone for the analysis that follows; 
several other datasets were used to account for confounding variables.
...
To supplement the backbone, various other datasets were gathered to form a stronger basis for explaining the stark gap in
the ride-hailing market. These datasets include:
...
\begin{enumerate}
    ...
    \item \textbf{TLC Monthly Data Report}~\cite{tlc_monthly_report} -- a separate published aggregate (trips-per-day by
    license class, 2009 onward), not the raw trip-record dataset above; used only for the 17-year market-shift context
    in Figure~\ref{fig:longrun}, since pulling raw per-trip records back to 2009 was not necessary for that purpose.
\end{enumerate}
...
\begin{itemize}
    \item[$\star$] Event proximity is engineered from the raw permit data (Section~\ref{sec:preprocessing}) into a per-zone-hour binary flag, \texttt{is\_event}: does this (zone, hour) overlap a major permitted event in its borough. Coarse as it is, it is a statistically significant demand driver (IRR $1.010$, $p=0.008$) once persistence and weather are controlled for.
...
pre-dawn trough (58 avg.\ yellow-taxi trips) to a Saturday-night peak (2{,}182), whilst taxi's own \textit{market share} moves on a different,
out-of-phase clock, peaking Tuesday evening (21.9\%) and troughing in the Tuesday small hours (4.2\%). Total demand and taxi's
relative competitiveness therefore follow a distinct \textbf{cyclical patter} within the week, motivating the harmonic features of
Section~\ref{sec:harmonics}.
...
Analysis of different weather scenarios shows Uber is the only mode with a significant temperature effect, hot days reduce only its
demand. Rain lifts all modes demand, yellow taxi proportionally most at $+8.5\%$ (Table~\ref{tab:weather_r}, Table~\ref{tab:rain_effect}).
...
Observed taxi volume is supply-censored ($Y^{taxi} = \min(\text{demand}, \text{available taxis})$), so total volume is the modelling target -- a cleaner proxy for addressable market. Counts are non-negative and heteroskedastic, so a GLM with a log link replaces OLS as the starting point; an initial Poisson fit was rejected in favour of \textbf{Negative Binomial (NB)} once overdispersion was confirmed ($\text{Var}(Y) \gg \mathbb{E}[Y]$), so NB, $\text{Var}(Y) = \lambda + \alpha\lambda^2$ ($\alpha$ via Cameron--Trivedi regression~\cite{cameron_trivedi_count}; nests Poisson at $\alpha=0$), is the specification used throughout:
\begin{equation}
    \log \lambda_{z,t} = \mathbf{x}_{z,t}^\top \boldsymbol\beta, \qquad Y_{z,t} \sim \text{NB}(\lambda_{z,t}, \alpha),
\end{equation}
guaranteeing $\lambda > 0$ and a multiplicative rate effect per covariate ($e^{\beta_j}$, an incidence rate ratio). Zone-hours also share unmodelled local conditions over time (an unflagged event, a driver shortage), so inference uses zone-clustered sandwich SEs.

\subsection{Harmonic (Fourier) Seasonality}
\label{sec:harmonics}

Hour-of-day dummies ignore circularity (hour 23 neighbours hour 0); a sinusoid is the natural basis instead. Each cycle is represented by $K$ summed daily/weekly/annual sine-cosine harmonic pairs $H_P^{(K)}(t)$, linearised via the angle-addition identity so they stay ordinary GLM coefficients rather than nonlinear parameters. Three blocks cover demand's natural cycles: $H(t) = H_{24}^{(2)}(t) + H_{168}^{(1)}(t) + H_{8766}^{(1)}(t)$ (daily, weekly, annual). Daily uses $K{=}2$ to separate AM/PM peaks; weekly/annual stay at $K{=}1$ -- one year of data cannot distinguish a higher annual harmonic from a secular trend.
...
The Poisson fit was strongly overdispersed (Pearson $\hat\phi = 5.98$), so results are reported from its NB refit ($\hat\alpha = 0.0315$), using the binary event-proximity encoding\footnote{The three candidate encodings (binary, count, $\log_1p$) scored within 0.001 deviance of each other; the binary indicator was kept as simplest.} (Table~\ref{tab:model_comparison}).
...
No taxi, $t{-}2$h           & 1.017 & [1.006, 1.028] & 0.002 \\
...
\begin{itemize}
\item \textbf{Every dollar figure is a ceiling, not a promised return.} \$827{,}773/hr assumes every uncaptured trip converts to a fare -- unrealistic, since some riders are app-native and unreachable regardless of availability. Realistic capture is scenario-based (\$82{,}772/\$165{,}543/\$248{,}315/hr at $10/20/30\%$), and Recommendation~\ref{rec:reallocate} reads the same way.
\item \textbf{This is a prioritisation tool, not a dispatch system.} The NB model carries roughly 17--20\% relative error in Manhattan (Section~\ref{sec:results}), enough to rank zone-hours, not drive minute-by-minute dispatch. Neither model says \emph{why} a zone under-performs (price, wait time, habit, competition).
...
\begin{enumerate}
\item \label{rec:reallocate} \textbf{Prioritise dispatch/positioning toward LaGuardia, JFK and the under-served Manhattan zones in Table~\ref{tab:uncaptured_demand}, not Brooklyn.} Ranked by uncaptured demand alone, the largest gaps are almost entirely Brooklyn (Crown Heights North, Bushwick, Williamsburg) -- but taxi holds only 1--3\% of trips there against an already-dominant market: unwinnable, not under-supplied. Restricting to zones taxi can realistically contest ($\geq10\%$ capture) instead points to the two airports and East Village/Times Square/Midtown Center -- real presence plus hundreds of uncaptured trips/hour each.

\item \textbf{Concentrate driver incentives on the specific hours taxi already loses, not general demand peaks.} Taxi's share of total demand moves out of phase with demand itself: it peaks at $21.9\%$ Tuesday evening and collapses to $4.2\%$ in the same day's small hours (Figure~\ref{fig:typical_week}), and falls below its seasonal-window average on every major holiday (Table~\ref{tab:holidays}), most sharply at Christmas ($11.3\%$ vs.\ $13.7\%$) -- exactly when rideshare's more elastic supply fills the gap. Shift bonuses timed to these windows are cheaper and faster to deploy than Recommendation 1, and can be trialled alongside it.

\item \textbf{Do not tie dispatch incentives to weather forecasts.} Bad weather reliably grows total demand -- every mode gains from rain, yellow taxi proportionally most (Table~\ref{tab:rain_effect}) -- but the binomial capture model finds no significant effect of rain, snow or thunderstorm on taxi's \emph{share} (all $p>0.17$); only temperature shifts the split (odds ratio $1.0033/^\circ$C). A rain push would grow a market taxi and rideshare already share proportionally -- Recommendations 1 and 2 are what actually move that split.
\end{enumerate}

\FloatBarrier
\section{Conclusion}

Rideshare took a large share of the ride-hailing market from taxi; this paper measures how much is realistically recoverable, and where. \textbf{Uncaptured Demand} decomposes market size from taxi's share via Poisson thinning: a citywide ceiling of \$827{,}773/hr, with a zone ranking honest about its limits -- Brooklyn's gaps reflect a market already lost, while Manhattan and the airports are worth contesting. The three recommendations act on three levers -- geographic, temporal, negative -- each a prioritisation signal, not a guarantee (Section~\ref{sec:limitations}).
